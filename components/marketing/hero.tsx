    ...
    <section className="relative overflow-hidden regroup-noise pt-28 pb-16 sm:pt-32 sm:pb-24">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        ...
        >
          ...
          <motion.h1
            variants={fadeUp}
            className="mt-4 text-4xl font-semibold tracking-tight text-foreground sm:text-5xl lg:text-[3.4rem] lg:leading-[1.08]"
          >
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[72px_1fr]">
        <div className="space-y-2 border-r border-border bg-[#F8FAFA] p-2">
          {["Pages", "Sections", "Theme", "Media"].map((item) => (
            <div
              key={item}
              className="rounded-lg bg-white px-2 py-2 text-[10px] font-medium text-stone-600 shadow-sm"
            >
              {item}
            </div>
          ))}
        </div>
        <div className="bg-[#EEF0F2] p-3">
          <div className="overflow-hidden rounded-xl border border-border bg-white">
            <div className="flex items-center justify-between border-b border-border px-3 py-2">
              <div className="flex items-center gap-2">
                <div className="h-6 w-6 rounded-md bg-brand" />
                <span className="text-[11px] font-semibold">Grace Community</span>
              </div>
            ...
            <div className="grid gap-3 p-3 sm:grid-cols-2">
              <div>
                ...
                <div className="mt-3 inline-flex rounded-lg bg-brand px-2.5 py-1 text-[10px] font-medium text-white">
                  Plan a Visit
                </div>
              </div>
              <div className="min-h-[88px] rounded-xl bg-gradient-to-br from-brand via-[#2A6F6A] to-accent" />
            </div>
            <div className="grid grid-cols-3 gap-2 border-t border-border bg-[#F8FAFA] p-3">
              {["Sunday Worship", "Youth Night", "Bible Study"].map((title) => (
                <div key={title} className="rounded-lg border border-border bg-white p-2">

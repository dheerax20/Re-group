...
const SYSTEM = [
  ...
  "primary, accent, inverted), textTone (default/muted/inverted/accent), radius ",
  "(none/sm/md/lg/xl/full) for rounding a band, a card or a button — `full` on a button is a ",
  "pill. `style` is merged, so set only the keys you are changing. ",
  ...
  "answer; only use `inverted` when they explicitly ask for a dark band. Never set a CONTENT band's ",
  "padding below `lg`, or its gap to `none` or `xs`. The `nav` and `footer` bands are the exception: ",
  "their height is fixed by the renderer and any padding you set on them is discarded, so don't. ",
  "RESTRAINT: don't add an `eyebrow` above a heading that already reads clearly — a small label above ",
  "every band is the fastest way this page starts looking generated. Don't add a `stats` block of ",
  "numbers the church has not given you. When you rename a button, name the action it performs ",
  "(\"Plan Your Visit\"), never \"Learn More\" or \"Get Involved\". ",
